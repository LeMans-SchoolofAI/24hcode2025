...
system_message = SystemMessage(content=
    f"""Tu es un concierge virtuel pour l'hôtel California. L'hôtel est situé dans la ville du Mans, en France.
       ...
       éviter des conflits.
       Quand tu demandes le type de repas pour une réservation, soit attentif à respecter la langue du client 
       pour ne pas lui donner des informations en anglais. Si le restaurant a déjà été choisi dans l'échange avec le 
       client alors tu seras également attentif à ne proposer que les types de repas réellement disponibles dans 
       ce restaurant (tu peux te fier aux horaires d'ouverture du restaurant). S'il n'y a qu'un seul type de repas 
       disponible alors tu éviteras de demander et tu proposeras directement ce type de repas pour confirmer que c'est 
       ok pour le client.
       Aujourd'hui nous sommes le {date}."""
)

# Fonction pour streamer l'audio généré par Kokoro
def stream_audio_response(text, voice='af_heart'):
    """
    Stream l'audio segment par segment pour réduire la latence
    """
    # Retrait des sauts de ligne dans le texte fournit car kokoro
    # interprete cela comme un nouveau segment audio et un changement d'interlocuteur
    # On remplace par des virgules pour qu'il y ait une pause dans la lecture
    text = text.replace('\n', ', ')

    # Placeholder pour afficher l'audio en cours
    audio_placeholder = st.empty()
    
    # Générer le pipeline Kokoro
    generator = pipeline(text, voice=voice)
    
    # Traiter chaque segment dès qu'il est disponible
    for i, (gs, ps, audio) in enumerate(generator):
        # Créer un buffer virtuel pour ce segment
        virtual_file = io.BytesIO()
        sf.write(virtual_file, audio, 24000, format='WAV')
        virtual_file.seek(0)
        
        # Afficher le segment dans le placeholder
        # Le premier segment commence à jouer immédiatement
        with audio_placeholder:
            st.audio(virtual_file, format='audio/wav', autoplay=True)
    ...
    with st.chat_message("assistant"):
        with st.spinner("En train de réfléchir..."):
            ...
            with st.spinner("Génération audio..."):
                stream_audio_response(response, voice='af_heart')
